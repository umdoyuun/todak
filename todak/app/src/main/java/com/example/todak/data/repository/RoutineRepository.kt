...
class RoutineRepository {
    ...
    suspend fun createRoutine(title: String, description: String, steps: List<RoutineStepRequest>): NetworkResult<RoutineCreateResponse> {
        Log.d(TAG, "createRoutine 호출됨 - title: $title")
        ...
        return try {
            ...
            // 요청 바디 생성
            val request = RoutineCreateUpdateRequest(
                title = title,
                description = description,
                steps = steps
            )

            Log.d(TAG, "API 요청 데이터: title=$title, steps=${steps.size}개")

            // API 호출
            val response = apiService.createRoutine(
                userId = userId,
                authorization = "Bearer $accessToken",
                request = request
            )
            ...
            when {
                response.isSuccessful -> {
                    response.body()?.let { result ->
                        Log.d(TAG, "API 호출 성공: ${result.message}, id=${result.id}")
                        NetworkResult.Success(result)
                    } ?: run {
                        Log.e(TAG, "응답 본문이 비어 있습니다")
                        NetworkResult.Error("응답 본문이 비어 있습니다")
            ...
            Log.e(TAG, "루틴 생성 오류", e)
            NetworkResult.Error("오류 발생: ${e.message}")
        }
    }

    suspend fun updateRoutine(routineId: String, title: String, description: String, steps: List<RoutineStepRequest>, isActive: Boolean): NetworkResult<RoutineActionResponse> {
        Log.d(TAG, "updateRoutine 호출됨 - routineId: $routineId, title: $title")
        ...
        return try {
            ...
            Log.d(TAG, "API 요청 데이터: routineId=$routineId, title=$title, steps=${steps.size}개, is_active=$isActive")

            // API 호출
            val response = apiService.updateRoutine(
                routineId = routineId,
                userId = userId,
                authorization = "Bearer $accessToken",
                request = request
